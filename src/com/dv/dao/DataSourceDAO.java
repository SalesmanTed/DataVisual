...
public class DataSourceDAO extends HibernateDaoSupport {
    ...
    public void addFileDataset(String datasetName, String fileName, String fileSize, String filePath, String separator, String hasheadline, String missing, String user) {
        ...
        fd.setFileStatus("1");
        System.out.println("fd"+fd.getId());
        try{
    ...
    public void addDbDataset(String dbtype,String dbname,String tablename){
    	 ...
    	 this.getHibernateTemplate().save(dd);
    }
    @SuppressWarnings("unchecked")
    public int getTotalFileDataset(String user) {
        String hql = "select count(*) from Filedataset where user='"+user+"'";
    ...
    @SuppressWarnings("unchecked")
    public List<Filedataset> fileDatasetList(final int pageno, final int pagesize, final String user) {
        List<Filedataset> allFileDataset = getHibernateTemplate().executeFind(new HibernateCallback() {
            public Object doInHibernate(Session session)
                    throws HibernateException, SQLException {
                String hql = "from Filedataset where fileStatus = '1' and user = '"+ user +"' order by uploadtime desc ";
