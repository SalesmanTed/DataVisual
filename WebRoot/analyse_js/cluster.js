...
function kmeans_analyse()
{
	...
	function(returnedData, status) {
		if("success"==status)
		{
			...
			//var obj=eval(returnedData);
			var existData = obj.existData;
			console.log(existData);
			var result = obj.result;
			var deleteData = obj.deleteData;
			var detailData="";
			if(deleteData.length!=0){
				for(var i=0;i<deleteData.length;i++){
					detailData+=deleteData[i]+"、";
				}
				flag=1;
				
			}
			detailData=detailData.substring(0, detailData.length-1);
			detailData+="变量不满足该分析的变量要求已排除在本次分析之外\n";
			console.log(detailData);
			console.log(result);
			
			if(result!="成功"&&result!="其他符合要求变量已成功加入分析算法中，结果如下"){
				
				var id = document.getElementById("result");
				id.innerHTML +="<p align='center'>该数据集不符合聚类分析要求，请重新选择数据集</p>";
				$("#main").unmask();
				alert(detailData+result);
				return ;
			}
			else{
			if(flag==0){
			...
			$("#result").append("<div id='scattermatrix' style='display:none;width:1000px;height:700px;align='center';text-align: center;'><div class='tabletitle'><div class='matrixHeadTitle' title='散点矩阵'>散点矩阵</div></div><div id='scattermatrixResult' style='margin-left:30%;'></div></div>"); 
...
//			$("#result").append(html);
			...
			$("#result").append("<div id='parallel' style='display:none'><div class='tabletitle'><div class='parallelHeadTitle' title='平行坐标图'>平行坐标图</div></div><div id='parallelResult' style='margin-left:10%;'></div></div>");
