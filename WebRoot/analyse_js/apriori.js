	...
	function(returnedData, status) {
		...
		{	
			...
			if(result!="成功"&&result!="其他符合要求变量已成功加入分析算法中，结果如下"){
				...
				id.innerHTML +="<p align='center'>分析无关联规则，请重新选择数据集</p>";
				$("#main").unmask();
				console.log(result);
				return ;
			}
		...
		//	$("#result").append("<div id='barchart' style='display:none;width:1000px;height:500px;margin:0 auto;'></div>");
			...
			html="";
			html=html+"<div align='center'><img width=1000px height=1000px  src='"+spath+"/rimages/"+imagename+"'></div>";
			$("#rulesimg").append(html);
			     
			
			$("#result").append("<div id='parallel' style='display:none'><div class='tabletitle'><div class='paraHeadTitle' title='平行坐标图'>平行坐标图</div></div><div id='parallelResult' style='margin-left:5%;'></div></div>");     
			//平行坐标系的绘制，多对多
