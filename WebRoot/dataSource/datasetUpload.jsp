...
<body>
	...
	<div id="content_template" class="content">
		<div>
			<div class='opr_title_style'>数据上传</div>
		...
		<div id="main">
			<form action="upload" method="post" id="form" enctype="multipart/form-data" class="form-horizontal">
				...
				<s:hidden name="user" id="user" value=""></s:hidden>
				<div class='tableList' style="width:50%;margin-left:30%;">
   					<div class='key_s'>数据集名称</div>
				    <div class='value_s' id="data">
				      <s:textfield id="datasetname" name="datasetname" value="" style="display:none" data-step="1" data-intro="定义你的数据集名称（如：购物车数据）" data-position='right'></s:textfield>
				    </div>
			    </div>
			    
			    <div class='tableList' style="width:50%;margin-left:30%;">
   					<div class='key_s'>上传文件的数据分隔符</div>
				    <div class='value_s'>
				      <select name="separatorselect" id="separatorselect" class="selectstyle"  data-step="2" data-intro="选择每行数据的分隔符（1,2,3分隔符为逗号），也可以自定义你的数据分隔符" data-position='right'>
			    		...
			    		<option value=",">,(csv默认数据分隔符)</option>
			    		<option value=";">;</option>
			    ...
			    <div class='tableList'  data-step="6" data-intro="从计算机中选择你要上传的文件，支持多文件上传" style="width:50%;margin-left:30%;">
	...
	<script type="text/javascript">
		...
		var dataName = time+"/"+name;
		document.getElementById("data").innerHTML+= dataName;
		document.getElementById("datasetname").value=dataName;
